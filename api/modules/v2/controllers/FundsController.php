...
class FundsController extends BaseController
{
    ...
    public function actionFundsguanli(){
        $new = '';
        $info = '';
        $fundguanli = new AwsGuNewGuanli();
        $guanli = $fundguanli::find()->asArray()->orderBy('gu_guanli_suo asc')->all();
        foreach($guanli as $v){
            $firstString = substr($v['gu_guanli_suo'],0,1);
            $new[$firstString][] = $v['gu_guanli_name'];
        }
        $i = 0;
        if($new){
    ...
    public function actionFundstuoguan(){
        $new = '';
        $info = '';
        $fundtuoguanli = new AwsGuNewTuoguan();
    ...
    public function actionAllcategory(){
        ...
        $categoryInfo = AwsGuNewCategory::find();
    ...
    public function actionDemo(){
        $test = [];
        $categoryInfo = AwsGuNewCategory::find()->where(['category_level'=>1])->asArray()->all();
        $insert = [];
        foreach($categoryInfo as $kk=>$v){
            $info = AwsGuNewInfo::find()->where(['gu_xifen2'=>$v['category_name']])->asArray()->all();
            foreach($info as $k=>$vv){


                                $insert[$kk][$k][] = $v['id'];
                $insert[$kk][$k][] = $vv['gu_xifen3'];
                $insert[$kk][$k][] = 2;
            }

        }

        foreach($insert as $vvv){
            Yii::$app->db->createCommand()->batchInsert('aws_gu_new_category', ['parent_id', 'category_name','category_level'], $vvv)->execute();
        }
       return $test;
    }
}
